...
public class RedisUtil<String,V> {
     ...
     * @return
     */
    public boolean set(final String key, V value, Long expireTime, TimeUnit timeUnit) {
        boolean result = false;
        try {
            ValueOperations<String, V> operations = redisTemplate.opsForValue();
            operations.set(key, value);
            ...
            //判断异常是否是Redis服务器连接
            if(e instanceof RedisConnectionFailureException) throw new RuntimeException("Redis服务器连接异常");
            e.printStackTrace();
        }
        return result;
    }
     ...
     * @param v
     */
    public void lPush(String k, V v) {
    ...
    public List<Object> lRange(String k, long l, long l1) {
        ListOperations<String, V> list = redisTemplate.opsForList();
        return (List<Object>) list.range(k, l, l1);
    }
     ...
     * @param scoure
     */
    public void zAdd(String key, V value, double scoure) {
     ...
     * @return
     */
    public Set<V> rangeByScore(String key, double scoure, double scoure1) {
        ZSetOperations<String, V> zset = redisTemplate.opsForZSet();
